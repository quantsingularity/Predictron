...
DEFAULT_ARTIFACT_DIR = Path(__file__).resolve().parents[2] / "inference_api" / "artifacts" / "price_direction"


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--candles", required=True, help="CSV path with OHLCV candles")
    parser.add_argument("--out", default=str(DEFAULT_ARTIFACT_DIR), help="Output artifact directory")
    args = parser.parse_args()

    df = pd.read_csv(args.candles, parse_dates=["timestamp"]).set_index("timestamp").sort_index()
