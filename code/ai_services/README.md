...
The principle behind this: an off-chain service (a webhook, an admin
panel, a model) should never become a trusted source of truth for money.
Adding an ML service is genuinely useful, but only if it's kept on the
read/advise side of that line permanently, not "for now."
